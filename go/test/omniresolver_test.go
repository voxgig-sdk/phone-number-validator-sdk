...
import (
	...
	omni "github.com/voxgig-sdk/phone-number-validator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PhoneNumberValidatorError); is {
				out := map[string]any{
					"name":    "PhoneNumberValidatorError",
...
func omniCtx(arg any, client *sdk.PhoneNumberValidatorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PhoneNumberValidatorSDK); is {
			client = live
