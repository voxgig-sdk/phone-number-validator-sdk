...
The Lua SDK for the PhoneNumberValidator API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:PhoneValidation(nil):load({ id = "example_id" }, nil)
...
local result, err = client:PhoneNumberValidator(nil):load(
  { id = "test01" }, nil
)
