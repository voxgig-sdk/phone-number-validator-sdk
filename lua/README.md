# PhoneNumberValidator Lua SDK



The Lua SDK for the PhoneNumberValidator API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:PhoneValidation()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local phonevalidation, err = client:PhoneValidation():load({ phone_number = "example" })
if err then error(err) end
```
...
local result, err = client:PhoneValidation():load({ phone_number = "example" })
...
```
lua/
├── phone-number-validator_sdk.lua    -- Main SDK module
...
The main module (`phone-number-validator_sdk`) exports the SDK constructor
