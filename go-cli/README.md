# phone-number-validator-cli

boru-driven command-line client **and** interactive REPL for the PhoneNumberValidator
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/phone-number-validator-cli)
make build

# 2. See usage (words, entities, env vars)
./phone-number-validator-cli --help

# 3. Provide credentials once, via the environment
export PHONE_NUMBER_VALIDATOR_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./phone-number-validator-cli load 1 phone_validation            # {id:1} shorthand
./phone-number-validator-cli load '{id:1}' phone_validation       # explicit match map

# 5. Override the API base URL for a single call
PHONE_NUMBER_VALIDATOR_BASE=https://api.example.com ./phone-number-validator-cli load 1 phone_validation

# 6. No arguments -> interactive REPL
./phone-number-validator-cli
phone-number-validator> load 1 phone_validation
phone-number-validator> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/phone-number-validator-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/phone-number-validator-cli load 1 phone_validation
   ```
...
```sh
./phone-number-validator-cli load 1 phone_validation          # scalar shorthand for {id:1}
./phone-number-validator-cli load '{id:1}' phone_validation     # explicit match map
```
...
./phone-number-validator-cli load 1 phone_validation
```
...
Run with no arguments to open a REPL (prompt `phone-number-validator>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `phone_validation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/phone-number-validator-cli`. |
...
(`./phone-number-validator-cli <expr>`) and interactively (the REPL), and expressions compose the
