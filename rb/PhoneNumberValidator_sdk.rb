...
require_relative 'PhoneNumberValidator_types'
...
class PhoneNumberValidatorSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PhoneNumberValidatorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PhoneNumberValidatorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PhoneNumberValidatorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => PhoneNumberValidatorError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue PhoneNumberValidatorError => err
    ...
    if fetched.is_a?(Hash)
      status = PhoneNumberValidatorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PhoneNumberValidatorError.new(
        "graphql_error", "PhoneNumberValidatorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PhoneNumberValidatorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
