...
class PhoneValidationEntity
  ...
  def make
    opts = @_entopts.dup
    PhoneValidationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PhoneValidation, Hash] the current PhoneValidation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PhoneValidation fields)
  def match_get
  ...
  #
  # @param reqmatch [PhoneValidationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PhoneValidation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PhoneValidation, Hash] the loaded PhoneValidation; raises PhoneNumberValidatorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PhoneNumberValidatorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
