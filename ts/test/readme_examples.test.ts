...
//     (\`new PhoneNumberValidatorSDK(...)\` / \`PhoneNumberValidatorSDK.test(...)\` become
//     \`PhoneNumberValidatorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'PhoneNumberValidatorSDK'
...
const TEST_SEED = {"entity":{"phone_validation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PhoneNumberValidatorSDK(...)\` and any
// \`PhoneNumberValidatorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PhoneNumberValidatorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PhoneNumberValidatorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
