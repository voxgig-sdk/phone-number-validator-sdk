...
describe('PhoneValidationDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new PhoneNumberValidatorSDK({
  ...
  test('direct-load-phone_validation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-phone_validation', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['PHONE_NUMBER_VALIDATOR_TEST_PHONE_VALIDATION_ENTID']
