...
The Python SDK for the PhoneNumberValidator API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.PhoneValidation(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.PhoneNumberValidator(None).load(
    {"id": "test01"}, None
)
