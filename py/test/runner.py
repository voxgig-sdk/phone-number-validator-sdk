# PhoneNumberValidator SDK test runner
...
class PhoneNumberValidatorTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PhoneNumberValidatorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PhoneNumberValidatorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PhoneNumberValidatorTestRunner.getenv("PHONE_NUMBER_VALIDATOR_TEST_LIVE")
        override = PhoneNumberValidatorTestRunner.getenv("PHONE_NUMBER_VALIDATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PhoneNumberValidatorTestRunner.getenv(key)
        ...
        explain = PhoneNumberValidatorTestRunner.getenv("PHONE_NUMBER_VALIDATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if PhoneNumberValidatorTestRunner._test_control is not None:
            return PhoneNumberValidatorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PhoneNumberValidatorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PhoneNumberValidatorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PhoneNumberValidatorTestRunner.load_test_control()
...
def load_env_local():
    PhoneNumberValidatorTestRunner.load_env_local()


def env_override(m):
    return PhoneNumberValidatorTestRunner.env_override(m)


def entity_data(v):
    return PhoneNumberValidatorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PhoneNumberValidatorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PhoneNumberValidatorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PhoneNumberValidatorTestRunner.load_test_control()


def live_client_options():
    return PhoneNumberValidatorTestRunner.live_client_options()


def live_delay_ms():
    return PhoneNumberValidatorTestRunner.live_delay_ms()
