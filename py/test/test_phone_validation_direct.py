...
class TestPhoneValidationDirect:

    def test_should_direct_load_phone_validation(self):
        setup = _phone_validation_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-phone_validation", "live" if setup["live"] else "unit")
        if _skip:
...
def _phone_validation_direct_setup(mockres):
