...
// PhoneNumberValidator SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class PhoneNumberValidatorTestFeature extends PhoneNumberValidatorBaseFeature
{
    ...
    public function init(PhoneNumberValidatorContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (PhoneNumberValidatorContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = PhoneNumberValidatorParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (PhoneNumberValidatorContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(PhoneNumberValidatorContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = PhoneNumberValidatorParam::call($ctx, $k);
