...
// PhoneNumberValidator SDK PhoneValidation entity
...
class PhoneValidationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PhoneValidationEntity($this->_client, $opts);
    }

    /**
     * @param PhoneValidation|array $args PhoneValidation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PhoneNumberValidatorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PhoneValidation|array The current PhoneValidation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PhoneValidation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PhoneNumberValidatorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PhoneValidation fields).
     */
    ...
    /**
     * Load a single PhoneValidation.
     *
     * @param PhoneValidationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PhoneValidationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PhoneValidation|array The loaded PhoneValidation as an assoc-array at the
     *   SDK boundary; throws PhoneNumberValidatorError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PhoneNumberValidatorHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
