...
class PhoneNumberValidatorMakeError
{
    public static function call(?PhoneNumberValidatorContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new PhoneNumberValidatorContext([], null);
        }
        $op = $ctx->op ?? new PhoneNumberValidatorOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new PhoneNumberValidatorResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof PhoneNumberValidatorError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new PhoneNumberValidatorError('', $msg, $ctx);
        ...
        if ($err instanceof PhoneNumberValidatorError) {
