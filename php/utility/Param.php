...
// PhoneNumberValidator SDK utility: param

require_once __DIR__ . '/../core/Helpers.php';

class PhoneNumberValidatorParam
{
    public static function call(PhoneNumberValidatorContext $ctx, mixed $paramdef): mixed
    {
        $point = $ctx->point;
        $spec = $ctx->spec;
        $match_val = $ctx->match;
        ...
        $akey = '';
        if ($point) {
            $alias_map = PhoneNumberValidatorHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'alias'));
            if ($alias_map) {
