...
//    only - a typed PhoneNumberValidatorContext there would make every ctx
...
final class PhoneNumberValidatorOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new PhoneNumberValidatorContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new PhoneNumberValidatorSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new PhoneNumberValidatorResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new PhoneNumberValidatorError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new PhoneNumberValidatorResponse($clean['response']);
    ...
    public static function err_from_map($m): ?PhoneNumberValidatorError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new PhoneNumberValidatorError(is_string($code) ? $code : '', $msg);
    }
