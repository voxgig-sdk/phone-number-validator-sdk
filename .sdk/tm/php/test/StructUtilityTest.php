...
// PhoneNumberValidator SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = PhoneNumberValidatorOmni::makeStructRunner(
    ...
    {
        return PhoneNumberValidatorOmni::objmodel(PhoneNumberValidatorOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return PhoneNumberValidatorOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return PhoneNumberValidatorOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(PhoneNumberValidatorOmni::structfix($now, Struct::undef()))
                        !== json_encode(PhoneNumberValidatorOmni::structfix($val, Struct::undef()))) {
