...
// PhoneNumberValidator SDK feature test
//
...
class FtCtrl extends PhoneNumberValidatorControl
{
...
class FtClient
{
    ...
    private PhoneNumberValidatorUtility $utility;

    public function __construct(PhoneNumberValidatorUtility $utility, array $options)
    {
    ...
    public function get_utility(): PhoneNumberValidatorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PhoneNumberValidatorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PhoneNumberValidatorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PhoneNumberValidatorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PhoneNumberValidatorConfig::make_feature($name);
    ...
    private static function buildUrl(PhoneNumberValidatorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PhoneNumberValidatorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PhoneNumberValidatorResponse($fetched);
        }
    ...
    private function fail(PhoneNumberValidatorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PhoneNumberValidatorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
