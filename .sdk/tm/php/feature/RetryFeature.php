...
// PhoneNumberValidator SDK retry feature
...
class PhoneNumberValidatorRetryFeature extends PhoneNumberValidatorBaseFeature
{
    ...
    public function init(PhoneNumberValidatorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (PhoneNumberValidatorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(PhoneNumberValidatorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(PhoneNumberValidatorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
