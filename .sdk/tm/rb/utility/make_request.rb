...
module PhoneNumberValidatorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PhoneNumberValidatorResponse.new({})
    ...
    if fetch_err
      ...
      response = PhoneNumberValidatorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PhoneNumberValidatorResponse.new(fetched)
    else
