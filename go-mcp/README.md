# phone-number-validator-mcp

[MCP](https://modelcontextprotocol.io) server exposing the PhoneNumberValidator SDK as
two agent tools — `phone-number-validator_list` and `phone-number-validator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/phone-number-validator-mcp)
make build

# 2. Provide credentials via the environment
export PHONE_NUMBER_VALIDATOR_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user phone-number-validator \
  -- /absolute/path/to/phone-number-validator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./phone-number-validator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// phone-number-validator_list: first page of records
{ "entity": "phone_validation" }
{ "entity": "phone_validation", "query": { } }

// phone-number-validator_load: one record by id
{ "entity": "phone_validation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/phone-number-validator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user phone-number-validator \
     -- "$PWD"/dist/*/phone-number-validator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `phone-number-validator_list` and `phone-number-validator_load` tools now appear
   in new sessions. Ask the agent to *"list phone_validation using phone-number-validator"*
   and it calls `phone-number-validator_list` with `{"entity":"phone_validation"}`.
...
```sh
export PHONE_NUMBER_VALIDATOR_APIKEY=sk_live_xxx            # API key
export PHONE_NUMBER_VALIDATOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `phone-number-validator_list` tool
...
### Call the `phone-number-validator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `phone-number-validator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `phone-number-validator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PHONE_NUMBER_VALIDATOR_APIKEY` | API key sent with every request. |
| `PHONE_NUMBER_VALIDATOR_BASE` | Optional override of the API base URL. |
...
```sh
./phone-number-validator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"phone-number-validator_load","arguments":{"entity":"phone_validation","query":{"id":1}}}}'
```
